...
An alternative normalization strategy for these situations will be described later in the `r Biocpkg("chipseqDB", "cbp.html#normalization-for-composition-biases", "CBP analysis")`.
...
Additional fields include `num.tests`, the total number of windows in the cluster; 
`num.up.logFC`, the number of windows with a DB log-fold change significantly greater than zero;
and `num.down.logFC`, the number of windows with a log-fold change significantly below zero.
...
Here, coordinates of DB regions are saved in BED format via `r Biocpkg("rtracklayer")`, using the log-transformed FDR as the score.
