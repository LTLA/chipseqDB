...
This is demonstrated below using the libraries in the NF-YA data set - see below for an explanation of the parameter values for `ext=` and `width=`.
...
A window is removed if the sum of counts across all libraries is below the specified `filter=`.
...
```{r ccf-histone, fig.cap="Cross-correlations in various histone mark data sets as a function of the delay between forward and reverse read positions."}}
n <- 1000
...
In Figure \@ref(fig:pe-frag), we might use a value of around 250 bp as it is larger than all fragment sizes.
...
pe.bam <- system.file("extdata", "ex1.bam", package="Rsamtools") # using an example BAM file.
out <- getPESizes(pe.bam)
hist(out$sizes, breaks=50, xlab="Fragment sizes (bp)", ylab="Frequency", main="")
...
```{r}
c(out$diagnostics, too.large=sum(out$sizes > 250))
```
