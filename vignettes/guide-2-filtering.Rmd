---
title: Filtering out low-abundance windows
author:
- name: Aaron T. L. Lun
  affiliation:
...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{03. Filtering windows}
...
Many of the low abundance windows in the genome correspond to background regions in which differential binding (DB) is not expected. 
Windows with low counts will not provide enough evidence against the null hypothesis to obtain sufficiently low $p$-values for DB detection. 
...
Filtering is valid so long as it is independent of the test statistic under the null hypothesis [@bourgon2010independent].
...
Row sums can also be used for datasets where the effective library sizes are not very different, 
or where the counts are assumed to be Poisson-distributed between biological replicates. 

The exact choice of filter threshold for the overall NB mean may not be obvious.
In particular, there is often no clear distinction in abundances between genuine binding and background events, 
e.g., due to the presence of many weak but genuine binding sites.
...
Some degree of arbitrariness is unavoidable when balancing these opposing considerations.
...
We compute the log-transformed NB mean for each window^[Also known as the "average abundance".] with the `aveLogCPM()` function from the `r Biocpkg("edgeR")` package.
...
This represents an arbitrarily small coverage below which we are unwilling to consider binding events.

```{r}
keep.size <- abundances > aveLogCPM(5, lib.size=mean(data$totals))
summary(keep.size)
```

A count-based filter becomes less effective as the library size increases.
...
Another approach is to to assume that only a certain proportion - say, 0.1% - of the genome is genuinely bound. 
This corresponds to the top proportion of high-abundance windows.
The total number of windows is calculated from the genome length and the spacing between windows used in `windowCounts()`. 
The `filterWindows()` function returns the ratio of the rank of each window to this total, where higher-abundance windows have larger ranks.
Users can then retain those windows with rank ratios above the unbound proportion of the genome.

```{r}
prop.stat <- filterWindows(data, type="proportion")
keep.prop <- prop.stat$filter > 0.999
sum(keep.prop)
@
...
This approach involves choosing a filter threshold based on the fold change over the level of non-specific enrichment.
...
(As the sizes of the regions used for read counting are different between bins and windows,
The average abundance of each bin must be scaled down to be comparable to those of the windows.)

With `type="global"`, the `filterWindows()` function returns the increase in the abundance of each window over the global background.
...
```{r}
global.stat <- globalWindows(data, background=binned, type="global")
keep.global <- global.stat$global > log2(3)
sum(keep.global)
```

The effect of this filtering is visualized in Figure \@ref(fig:global-filt).
...
```{r global-filt, fig.cap="Distribution of average abundances across 2 kbp genomic bins. The median estimate of the global background abundance is shown in red, while the filter threshold is shown in blue."}
...
Local background estimators avoid inappropriate filtering when there are differences in background coverage across the genome. 
Here, the 2 kbp region surrounding each window is used as the "neighborhood" over which a local estimate of non-specific enrichment for that window can be obtained. 
...
This entire procedure is performed automatically by setting `type="local"` for `filterWindows()`, which returns the enrichment values i.e., the increase in the abundance of each window over its neighborhood.

```{r}
local.stat <- filterWindows(data, wider, type="local")
```
...
This roughly mimics the behavior of single-sample peak-calling programs such as `MACS` [@zhang2008macs].
...
This may be problematic for diffuse histone marks or clusters of transcription factor binding sites, 
where enrichment may be observed in both the window and its neighborhood.

## Identifying local maxima

Another approach uses the `findMaxima()` function to identify local maxima in coverage across the genome.
...
```{r}
keep.max <- findMaxima(rowRanges(data), range=1000, metric=abundances)
summary(keep.max)
```
...
Negative controls in ChIP-seq experiments refer to samples where the immunoprecipitation step has been skipped (input control) or compromised with an irrelevant antibody (IgG control), respectively. 
This allows us to model variation in coverage across the genome due to sequencing/mapping biases in ChIP-seq data. 
...
```{r}
with.input <- c(bam.files, "input.bam")
in.demo <- windowCounts(with.input, ext=frag.len, param=param)
chip <- in.demo[,1:4]
control <- in.demo[,5]
```

Some additional work is required to account for composition biases that are likely to be present when comparing ChIP to negative control samples (see later).
...
The `filterWindows()` function computes the enrichment of the ChIP counts over the control counts for each window.
This uses `scale.info=` to adjust for composition biases between ChIP and control sampes.
...
By comparison, the global and local background estimates require less protection (`prior.count=2`, by default) as they are derived from larger regions with more counts.

```{r}
control.stat <- filterWindows(chip, control, type="control", 
...
<<>>=
keep.control <- control.stat$filter > log2(3)
sum(keep.control)
@
...
The example below retrieves the coordinates of the broad gene bodies from the mouse genome, including the 3 kbp region upstream of the transcription start site that represents the putative promoter region for each gene. 
...
Windows are filtered to only retain those which overlap with the regions of interest. 
...
<<>>=
suppressWarnings(keep.prior <- overlapsAny(rowRanges(data), broads))
sum(keep.prior)
@
...
Peak calls from the current dataset may be used provided that peak calling was done independently of the DB status of each window. 
If libraries are of similar size and/or biological variability is low, we recommend pooling reads into one library for single-sample peak calling [@lun2014denovo].
...
For the purposes of our NF-YA analysis, we will use the global filter defined in `keep.global`.
The filter itself is applied simply by subsetting our `RangedSummarizedExperiment` object to retain only the rows of interest.
It is usually wise to assign the filtered object to a new variable, just in case the original `data` is needed later on.
...
Different filters can also be combined in more advanced applications, e.g., `data[keep1 & keep2,]` for filter vectors `keep1` and `keep2`.
