...
author: Aaron T. L. Lun, Gordon K. Smyth
author_affiliation: The Walter and Eliza Hall Institute of Medical Research, 1G Royal Parade, Parkville, VIC 3052, Melbourne, Australia; Department of Medical Biology, The University of Melbourne, Parkville, VIC 3010, Melbourne, Australia
date: 22-08-2015
output: 
...
biocbuild <- TRUE # BioC build starts from BAM files, not SRA.
```
...
These are counted into sliding windows with `r Biocpkg("csaw")`, to quantify binding intensity across the genome [@lun2014denovo].
...
Reads in each library are aligned to the mm10 build of the mouse genome, using the `r Biocpkg("Rsubread")` package [@liao2013subread].
...
```{r, results="hide", eval=!biocbuild, message=FALSE}
library(Rsubread)
...
```{r, message=FALSE, results="hide", eval=!biocbuild}
library(Rsamtools)
...
```{r, eval=biocbuild, results="hide", echo=FALSE, message=FALSE}
library(Rsubread)
library(Rsamtools)
...
```{r, eval=!biocbuild, results = 'hide'}
indexBam(bam.files)
```

```{r, echo = FALSE, results = 'hide', eval=!biocbuild}
# Cleaning up
unlink(all.fastq)
unlink(group.fastq)
...
All reads in the blacklist will be ignored during processing in `r Biocpkg("csaw")`.
...
```{r, eval=biocbuild, message=FALSE, echo=FALSE, results="hide"}
...
```{r trendplot, fig.cap="**Figure 3:** Abundance-dependent trend in the log-fold change between two H3K9ac libraries, across all retained windows. The red line represents a fitted loess curve."}
...
`r Biocpkg("csaw")` implements a version of the fast loess method [@ballman2004fast] that is adapted to count data.
...
```{r normplot, fig.cap="**Figure 4:** Effect of non-linear normalization on the trended bias between two H3K9ac libraries. Log-fold changes for all windows are shown after normalization. The red line represents a fitted loess curve."}
...
The `RangedSummarizedExperiment` object is coerced into a `DGEList` object (plus offsets) prior to entry into `r Biocpkg("edgeR")`.
...
```{r bcvplot, fig.cap="**Figure 5:** Abundance-dependent trend in the BCV for each window, represented by the blue line. Common and tagwise estimates are also shown."}
plotBCV(y)
```
...
The behaviour of `r Biocpkg("ChIPpeakAnno")` complements that of  `detailRanges`.
...
```{r, eval=biocbuild, echo=FALSE, results="hide"}
unlink(bam.files)
...
```{r, eval=!biocbuild, message=FALSE, results="hide"}
...
```{r, echo=FALSE, results="hide", eval=biocbuild, message=FALSE}
bam.files <- paste0(sra.numbers, ".bam")
for (bam in bam.files) {
    bam.url <- paste0(core.loc, bam)
    download.file(bam.url, bam)
...
```{r tfplot, fig.height=6, fig.width=8, fig.cap="**Figure 12:** Coverage tracks for TF binding sites that are differentially bound in the WT (top two tracks) against the KO (last two tracks). Blue and red tracks represent forward- and reverse-strand coverage, respectively, on a per-million scale (capped at 5 in SRR1145788, for visibility)."}
collected <- list()
lib.sizes <- filtered.data$totals/1e6
for (i in 1:length(bam.files)) { 
    reads <- extractReads(bam.file=bam.files[i], cur.region, param=param)
